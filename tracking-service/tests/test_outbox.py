...
(`telemetry.aggregated`…) para siempre. Ahora cada fallo reprograma la fila
con backoff exponencial y, al superar el umbral, solo alarma (log ERROR +
métrica) sin dejar de reintentar.
"""
...
METRICA_AGOTADOS = "tracking_outbox_eventos_agotados_total"
...
async def test_evento_agotado_sigue_reintentandose_con_backoff():
    ...
    async with SessionLocal() as session:
        fila = registrar_evento(session, "telemetry.aggregated", {"vehicle_id": "v-001"})
        await session.commit()
        id_fila = fila.id

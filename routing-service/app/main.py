"""Routing Service — optimización y asignación de rutas.

Responsabilidad única: qué vehículo lleva cada carga y con qué ruta, en el
menor tiempo posible. Arranca la API REST, el relay del outbox y el
consumidor del bus.
...
from app.routers import bus_interno, rutas, salud
...
def crear_app() -> FastAPI:
    settings = get_settings()
    app = FastAPI(
        title="LogiTrack · Routing Service",
        version=settings.version,
        description=(
            "Optimización y asignación de rutas. Consume shipment.created, "
            "telemetry.aggregated y vehicle.status_changed; publica "
            "route.assigned, route.recalculated y route.unassignable."
        ),
        lifespan=lifespan,
        docs_url="/docs",
    ...
    # Orígenes permitidos: de settings.cors_origins (frontend en 5173 y
    # panel.html servido en 4173). Lista explícita; antes "*".
    app.add_middleware(
        CORSMiddleware,
    ...
    if settings.demo_bus_interno and settings.entorno not in ENTORNOS_DONDE_NO_HAY_BUS_INTERNO:
        ...
        log(
            ...
            aviso="endpoint noexcept previsto para demos; no exponer en producción",
        )
    return app

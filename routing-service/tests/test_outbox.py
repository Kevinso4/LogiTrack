...
(`route.assigned`…) para siempre. Ahora cada fallo reprograma la fila con
...
METRICA_AGOTADOS = "routing_outbox_eventos_agotados_total"
...
async def test_evento_agotado_sigue_reintentandose_con_backoff():
    ...
    async with SessionLocal() as session:
        fila = registrar_evento(session, "route.assigned", {"ruta_id": "r-1"})
        await session.commit()
        id_fila = fila.id
